...
def build_context_messages(
    ...
    *,
    ...
    maximum = max(8_192, min(int(config.get("model_context_window_tokens", 128_000)), 2_000_000))
    reserve = max(512, min(int(config.get("context_output_reserve_tokens", 8_192)), maximum // 2))
    ...
    if enabled and utilization >= trigger_percent and len(segments) > minimum_recent:
        ...
        if prefix_count > 0:
            ...
            try:
                ...
                overrides = dict(summary_config.get("request_body_overrides", {}))
                overrides.setdefault("max_tokens", summary_budget)
                summary_config["request_body_overrides"] = overrides
