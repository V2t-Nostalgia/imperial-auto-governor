...
CONSOLE_UNIT_SOURCE="$SOURCE_ROOT/deploy/systemd/iag-agent-console.service"
CONSOLE_UNIT_TARGET="/etc/systemd/system/iag-agent-console.service"
OBSERVER_UNIT_SOURCE="$SOURCE_ROOT/deploy/systemd/iag-agent-network-observer.service"
...
if [[ "$RUNTIME_ROOT$VENV_ROOT$SERVICE_HOME$XAUTHORITY_VALUE" =~ [[:space:]] ]]; then
...
"$VENV_ROOT/bin/python" - \
  ...
  "$RUNTIME_ROOT" "$VENV_ROOT" "$SERVICE_USER" "$SERVICE_GROUP" \
