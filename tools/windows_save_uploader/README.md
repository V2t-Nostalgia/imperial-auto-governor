...
9. Agent 收到 READY 后才点击与动作同族的载体：川陀上的建设指令中继、发电区划、工程学研究特化、升级指令中继和替换指令中继分别用于建筑、主区划、区划特化、建筑升级和建筑替换。房主桥只改写该入站记录，并保持 UDP payload 长度、命令记录长度、命令数和 serial 不变。Host Bridge 9 在升级过渡期仍接受已实测的旧研究实验室、全息影像剧场等载体。
