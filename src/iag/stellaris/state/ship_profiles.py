...
def _design_profile(
    ...
    block: str,
    *,
    automatic_design_enabled: bool | None = None,
) -> dict[str, Any]:
    ...
    hidden_autogenerated = auto_generated and automatic_design_enabled is False
    return {
        "design_id": design_id,
        ...
        "player_visible": not hidden_autogenerated,
        "visibility": (
            "hidden_disabled_autogenerated"
            if hidden_autogenerated
            else (
                "active_autogenerated"
                if auto_generated
                else "player_visible_saved_design"
            )
        ),
        ...
        "model_clone_source_eligible": (
            not hidden_autogenerated and not obsolete and clone_supported
        ),
    }
...
def extract_ship_profiles(
    ...
    *,
    ...
    automatic_design_setting = bare_scalar(design_collection, "auto_gen_design")
    automatic_design_enabled = {
        "yes": True,
        "no": False,
    }.get(automatic_design_setting)
    design_ids = integer_values(design_collection, "ship_design")
    design_blocks = (
        state_index.numeric_map("ship_design")
        if state_index is not None
        else parse_numeric_map(find_braced_section(text, "ship_design").strip())
    )
    designs = [
        _design_profile(
            design_id,
            block,
            automatic_design_enabled=automatic_design_enabled,
        )
    ...
    try:
        known_technologies = set(
            (research_profile or extract_research_profile(
                text,
                owner=owner,
    ...
    systems = _system_index(text, state_index=state_index)
    ...
    return {
        ...
        "owner_country_id": owner,
        "automatic_design_enabled": automatic_design_enabled,
        "country_design_record_count": len(designs),
        "player_visible_design_count": sum(
            design.get("player_visible", True) for design in designs
        ),
        "hidden_autogenerated_design_count": sum(
            design.get("player_visible") is False for design in designs
        ),
        "designs": designs,
...
def _owned_design(profile: dict[str, Any], design_id: int) -> dict[str, Any]:
    ...
    if source.get("player_visible") is False:
        raise ValueError(
            f"Ship design {design_id} is a hidden autogenerated template while "
            "automatic ship design is disabled. Choose a player-visible design."
        )
