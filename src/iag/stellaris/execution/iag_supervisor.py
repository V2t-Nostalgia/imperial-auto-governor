...
def session_proxy_action_target(
    action: dict[str, Any],
) -> tuple[str, dict[str, Any]]:
    """Convert one validated economy action into a proxy request target."""
    ...
    if action_type == "build_building":
        ...
        target = {
            ...
            "build_queue_id": int(action["build_queue_id"]),
            "colony_id": int(action["colony_id"]),
            "district_type": str(action["district_type"]),
        }
...
def execute_run(
    ...
    if execution_mode == "session_proxy":
        ...
        try:
            proxy_result = controller.arm_and_wait(
                action=action_type,
                target=target,
            ...
            raise SupervisorError(str(error)) from error
        success = proxy_result.get("outcome") == "confirmed"
        result = {
            "schema": "iag.execution_result.v1",
            "success": success,
            "source": "session_proxy",
            "started_at": proxy_result.get("finished_at"),
            "finished_at": proxy_result.get("finished_at") or now_iso(),
            "confirmation_state": (
                "confirmed_by_packet" if success else "failed"
            ),
            "telemetry": {
                "carrier_seen": True,
                "rewritten": True,
                "authoritative_confirmation": success,
                "host_acknowledged_inserted_bytes": proxy_result.get(
                    "host_acknowledged_inserted_bytes"
                ),
                "proxy_request": proxy_result,
            },
            "source_save": str(source_save),
        }
        atomic_write_json(run_dir / "execution_result.json", result)
        if not success:
            raise SupervisorError(
                str(proxy_result.get("error") or "代理动作未获房主权威确认。")
            )
        return result
    if execution_mode != "carrier_click":
        raise SupervisorError(
            "execution_mode must be carrier_click or session_proxy."
        )
...
def main() -> int:
    args = parse_args()
    config = RuntimeConfig.load(args.config).snapshot().settings
    result = execute_run(args.run_dir, config)
