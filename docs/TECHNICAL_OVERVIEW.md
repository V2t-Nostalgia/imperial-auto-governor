...
同一源码状态生成四个独立归档：完整公开源码、Windows Agent、Windows Host Bridge 和 Ubuntu Agent。每个归档都带独立 manifest 与包内 SHA-256 清单；顶层 `SHA256SUMS-0.5.6.txt` 再覆盖四个归档。发布验证会对实际压缩包执行路径穿越、符号链接、禁止文件、凭据、邮箱、个人路径、真实地址、文件列表与哈希一致性检查，并在解压目录重新运行对应测试或健康检查。
