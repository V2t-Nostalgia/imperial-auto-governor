...
- 科研开始与停止；
- 船坞直接建造与舰船设计；
- Fleet Manager 模板创建、目标数量增减和两阶段增援。

当前共登记 24 个动作。每次增加新命令时，必须同时增加目录项和离线 fixture；仓库测试
会检查目录与代理动作是否一一对应。套件直接测试协议执行器，不等同于把动作开放给
LLM：攻击、殖民、民用舰船自动化和恒星基地操作仍需各自的存档候选与 Application
状态验证器。自动化取消 `6836` 因字段语义未定而不在目录中。
