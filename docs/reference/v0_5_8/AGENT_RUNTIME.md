...
工程化平台会把 API Key 随对应端点写入本机、未纳入 Git 的运行配置
`agent_config.json`。公开状态接口只返回 `api_key_configured`，不会把密钥
回显给浏览器。运行配置应限制为当前用户可读，并不得复制进日志、示例配置或
发布包。旧版单密钥文件和 `IAG_LLM_API_KEY` 环境变量不再是当前模型池的默认
配置路径。
