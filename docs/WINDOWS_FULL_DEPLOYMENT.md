...
6. 在“执行与确认”中设置规划存档版本容差。默认 2 允许正在运行的批次最多落后两份新上传存档；设为 0 可恢复严格最新。固定点击界面校验默认开启，只有在人工确认窗口正确且模板分数轻微波动时才关闭。
...
6. 让一轮巡检跨过至少一个自动存档点，确认新档不会打断容差范围内的既有批次；若同时跨过复查月份，前端应把下次复查滚到之后的周期边界，而不是立刻补跑过期轮次。
...
- **Stellaris 已运行但端口显示“待载体”**：Windows 套接字表可能只显示多个无对端 UDP 端口。只要房主执行桥已认证，执行器会按合作端 IP 安全待命，并在第一条真实载体命令到达时锁定本局端口；不得从多个端口中猜测。
