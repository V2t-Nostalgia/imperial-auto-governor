...
DEFAULT_RUNTIME_ROOT = (
    Path(os.environ.get("LOCALAPPDATA", Path.home())) / "ImperialAutoGovernor"
)
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise TypeError(f"{path} 必须包含 JSON 对象。")
    return value
...
def ensure_certificate(certificate_path: Path, key_path: Path) -> str:
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(
                NameOID.COMMON_NAME, "Imperial Auto Governor Local Console"
            )
        ]
    )
...
def server_command(
    config_path: Path, host: str, port: int, cert: Path, key: Path
) -> list[str]:
    arguments = [
        "--serve",
        "--config",
...
def run_health_check() -> int:
    ...
    from iag.core.paths import (
        economy_governance_root,
        fleet_operations_root,
        research_strategy_root,
        vanilla_content_pack_root,
    )
    ...
    from iag.stellaris.state import (
        extract_game_state,  # noqa: F401
        planet_profiles,  # noqa: F401
    )
...
class Launcher:
    ...
    def _style(self) -> None:
        ...
        style.configure(
            "Card.TFrame", background="#0b2427", relief="solid", borderwidth=1
        )
        style.configure(
            "TLabel",
            background="#071619",
        ...
        style.configure(
            "Gold.TLabel", foreground="#d9b95b", font=("Microsoft YaHei UI", 10, "bold")
        )
    ...
    def _layout(self) -> None:
        ...
        self.ttk.Label(frame, text="灰风内政执行端", style="Title.TLabel").grid(
            row=0, column=0, columnspan=3, sticky="w"
        )
        self.ttk.Label(
            frame,
            text="Windows co-op client · save analysis · guarded carrier click",
            style="Gold.TLabel",
        ...
        for index, (label, variable, command) in enumerate(labels, start=2):
            ...
            self.ttk.Entry(frame, textvariable=variable).grid(
                row=index, column=1, sticky="ew", padx=(12, 8), pady=6
            )
            if command:
                self.ttk.Button(frame, text="浏览", command=command).grid(
                    row=index, column=2, sticky="ew", pady=6
                )

        button_row = self.ttk.Frame(frame)
        button_row.grid(row=6, column=0, columnspan=3, sticky="ew", pady=(16, 10))
        self.ttk.Button(button_row, text="保存设置", command=self.save_settings).pack(
            side="left", padx=(0, 8)
        )
        self.start_button = self.ttk.Button(
            button_row, text="启动灰风", style="Accent.TButton", command=self.start
        )
        self.start_button.pack(side="left", padx=8)
        self.stop_button = self.ttk.Button(
            button_row, text="停止", command=self.stop, state="disabled"
        )
        self.stop_button.pack(side="left", padx=8)
        self.ttk.Button(button_row, text="打开控制台", command=self.open_console).pack(
            side="left", padx=8
        )

        self.ttk.Label(frame, textvariable=self.status_var, style="Gold.TLabel").grid(
            row=7, column=0, columnspan=3, sticky="w", pady=(4, 4)
        )
        self.ttk.Label(frame, textvariable=self.credentials_var, wraplength=790).grid(
        ...
        self.ttk.Button(
            credential_row,
            text="复制前端密码",
            command=lambda: self._copy_secret("frontend_password"),
        ).pack(side="left", padx=(0, 8))
        self.ttk.Button(
            credential_row,
            text="复制桥接令牌",
            command=lambda: self._copy_secret("save_upload_token"),
        ).pack(side="left", padx=8)
        self.ttk.Button(
            credential_row, text="复制证书指纹", command=self._copy_fingerprint
        ).pack(side="left", padx=8)
    ...
    def _browse_game(self) -> None:
        value = self.filedialog.askdirectory(
    ...
    def _config_template(self) -> dict[str, Any]:
        ...
        if not template.is_file():
            template = (
                Path(__file__).resolve().parent / "agent_config.windows.example.json"
            )
    ...
    def save_settings(self) -> dict[str, Any] | None:
        try:
            ...
            return config
        except Exception as error:  # noqa: BLE001 - GUI boundary reports all failures.
    ...
    def _refresh_credentials(self, config: dict[str, Any]) -> None:
        ...
        fingerprint = (
            certificate_fingerprint(certificate) if certificate.is_file() else "未生成"
        )
    ...
    def _copy_secret(self, name: str) -> None:
        try:
            self._copy(
                (self.runtime_root / "secrets" / name)
        ...
        try:
            self._copy(
                certificate_fingerprint(
                    self.runtime_root / "secrets" / "frontend_tls.crt"
                )
            )
        except FileNotFoundError:
            self.messagebox.showerror(APP_TITLE, "请先保存设置。")
    ...
    def start(self) -> None:
        ...
        self.server_log_offset = (
            server_log.stat().st_size if server_log.is_file() else 0
        )
