...
from apps.host_bridge.iag_host_interceptor import (
    is_windows_admin,
    packet_filter_for_routes,
)
from apps.host_bridge.iag_save_uploader import (
    APP_VERSION,
    PinnedHTTPSUploader,
    SaveUploader,
    UploaderError,
...
def default_config() -> dict:
    ...
    return {
        ...
        "heartbeat_seconds": 5,
        "overlay": {
            "access_token_file": "secrets/overlay_access_token",
...
class UploaderGUI:
    ...
    def launch_overlay(self) -> None:
        if self.overlay_process is not None and self.overlay_process.poll() is None:
            messagebox.showinfo("灰风游戏叠加层", "游戏叠加层已经在运行。")
            return
        if not self.save_settings(silent=True):
            return
        if getattr(sys, "frozen", False):
            candidates = (
                APP_ROOT / "overlay" / "IAGOverlay" / "IAGOverlay.exe",
                APP_ROOT / "overlay" / "IAGOverlay.exe",
                APP_ROOT / "IAGOverlay.exe",
            )
            executable = next((path for path in candidates if path.is_file()), None)
            if executable is None:
                messagebox.showerror(
                    "无法启动游戏叠加层",
                    "房主执行桥包中缺少 IAGOverlay.exe，请重新下载完整发布包。",
                )
                return
            command = [str(executable), "--config", str(self.config_path)]
        else:
            command = [
                sys.executable,
                "-m",
                "apps.game_overlay.main",
                "--config",
        ...
        try:
            ...
            self.append_log("已启动游戏叠加层。使用配置热键进入或退出输入模式。")
        except OSError as error:
            messagebox.showerror("无法启动游戏叠加层", str(error))
